...
def query_s3_all(data: QueryInput):
    ...
    if ec2 == 1 and s3 == 0:
        ...
        res = perform_query(android_id, s_date_time_input,
                            e_date_time_input, last_thursday)
...
def perform_query(android_id, s_date_time_input, e_date_time_input, last_thursday):
    ...
    time_interval_in_cache = requests.post(
        time_interval_in_cache_url, json=time_interval_in_cache_obj)
...
def query_app_usage_by_time_s3(android_id, s_date_time_input, e_date_time_input):
    ...
    remove_cache_metadata_duplicate_url = server_url + \
        ":3000/rpc/remove_cache_metadata_duplicate"
    remove_cache_metadata_duplicate_obj = {
        "device_id": android_id
    }
    remove_cache_metadata_duplicate = requests.post(
        remove_cache_metadata_duplicate_url, json=remove_cache_metadata_duplicate_obj)


def calculate_eviction_time(e_date_time_input):
    e_date_time = datetime.strptime(
        e_date_time_input.replace("T", " "), "%Y-%m-%d %H:%M:%S")
    now = datetime.now()
    date_time_now = now.strftime("%Y-%m-%d %H:%M:%S")
    now = datetime.strptime(date_time_now, "%Y-%m-%d %H:%M:%S")
